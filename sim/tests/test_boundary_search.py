"""
Tests for sim/boundary_search.py (Chunk 5: the boundary optimizer).
...
import boundary_search  # noqa: E402
...
def test_solo_foraging_payoff_takes_the_most_preferred_territory():
    ...
    assert boundary_search.solo_foraging_payoff(preferences, round_utilities, fresh_rng()) == pytest.approx(9.0)
...
def test_solo_foraging_truth_equals_best_of_three():
    ...
    engine = boundary_search.solo_foraging_payoff(truth, round_utilities, fresh_rng())
...
def test_solo_foraging_all_ties_equals_mean_territory_worth():
    ...
    engine = boundary_search.solo_foraging_payoff(flat_preferences, round_utilities, fresh_rng(3))
...
def test_optimize_boundaries_returns_the_argmax():
    ...
    for boundaries in candidates:
        ...
        independent.append(boundary_search.solo_foraging_payoff(prefs, round_utilities, fresh_rng(0)))
    expected_winner = candidates[int(np.argmax(independent))]

    result = boundary_search.optimize_boundaries(competition.cr3_preferences, candidates, foraging_rounds, round_utilities)
...
def test_optimize_boundaries_is_deterministic():
    ...
    a = boundary_search.optimize_boundaries(competition.cr3_preferences, candidates, foraging_rounds, round_utilities)
    b = boundary_search.optimize_boundaries(competition.cr3_preferences, candidates, foraging_rounds, round_utilities)
...
def test_cr3_candidate_boundaries_are_all_ordered_pairs_in_range():
    ...
    candidates = boundary_search.cr3_candidate_boundaries()
...
def test_if3_symmetric_candidates_are_symmetric_and_ordered():
    ...
    """
    candidates = boundary_search.if3_symmetric_candidate_boundaries()
